...
public abstract class DeobfExtension {
    ...
    public DeobfExtension(Project project) {
        ...
        dependencies.getArtifactTypes()
                .named(ArtifactTypeDefinition.JAR_TYPE, type -> type.getAttributes().attribute(DeobfAttributes.DEOBFUSCATED, DeobfAttributes.NONE));
        ...
        var renamerTask = Tasks.register(project, "deobfRenamerClasspath", RenameJar.class, project.getExtensions().getByType(RenamerExtension.class));
        ...
        dependencies.registerTransform(DeobfuscateJar.class, spec -> {
            spec.getFrom().attribute(DeobfAttributes.DEOBFUSCATED, DeobfAttributes.NONE).attribute(ArtifactTypeDefinition.ARTIFACT_TYPE_ATTRIBUTE, ArtifactTypeDefinition.JAR_TYPE);
            spec.getTo().attribute(DeobfAttributes.DEOBFUSCATED, DeobfAttributes.MCP).attribute(ArtifactTypeDefinition.ARTIFACT_TYPE_ATTRIBUTE, ArtifactTypeDefinition.JAR_TYPE);
        ...
        dependencies.registerTransform(ExtractUserdevDeobfInputs.class, spec -> {
            ...
            spec.getTo().attribute(ArtifactTypeDefinition.ARTIFACT_TYPE_ATTRIBUTE, DeobfAttributes.USERDEV_INPUTS_TYPE);
        });
    }

    public void useUserdev(Configuration userdev) {
        getUserdevInputs().from(
                userdev.getIncoming()
                        .artifactView(view -> view.getAttributes()
                                .attribute(ArtifactTypeDefinition.ARTIFACT_TYPE_ATTRIBUTE, DeobfAttributes.USERDEV_INPUTS_TYPE))
                        .getFiles()
        );
    }

    public void wireTransformOrdering(Project project) {
        var ordering = project.getDependencyFactory().create(project.files().builtBy(getMappings(), getSrgLibraries(), getRenamerClasspath()));
        ...
        project.getConfigurations().configureEach(configuration -> configuration.withDependencies(dependencies -> {
            var deobfDeclared = dependencies.stream().anyMatch(dependency -> dependency instanceof ExternalModuleDependency module && DeobfAttributes.MCP.equals(module.getAttributes().getAttribute(DeobfAttributes.DEOBFUSCATED)));
        ...
        compileClasspath.withDependencies(_ -> {
            for (var dependency : compileClasspath.getAllDependencies()) {
                if (dependency instanceof ExternalModuleDependency module &&
                    ...
                    var message = "deobf(" + module.getGroup() + ":" + module.getName() + ") cannot be declared on the '" + compileClasspath.getName() +
                            "' hierarchy in loader mode, because remapping it needs the " + "SRG-named Cleanroom jar that is built from this project's own compile classpath.";
                    throw CleanroomProblems.throwing(
                            problems,
                            new InvalidUserDataException(message),
                            CleanroomProblems.DEOBF_ON_COMPILE_CLASSPATH,
                            message,

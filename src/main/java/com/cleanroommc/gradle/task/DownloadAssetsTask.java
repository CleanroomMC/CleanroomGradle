...
//Copied from rfg https://github.com/GTNewHorizons/RetroFuturaGradle
public abstract class DownloadAssetsTask extends DefaultTask {
     ...
     */
    @InputFiles
    public abstract Collection<File> getManifests();
    ...
    public void downloadAssets() throws IOException {
        ...
        for (File manifest : getManifests()) {
            AssetIndexObjects assetsManifest;
            try (FileReader reader = new FileReader(manifest)) {
                assetsManifest = getGson().get().fromJson(reader, AssetIndexObjects.class);
            }

            List<AssetIndexObjects.Object> assets = assetsManifest.getObjectList();
            toDownload.set(assets.size());
            downloaded.set(0);
            for (var asset : assets) {
                queue.submit(AssetAction.class, action -> {
                    try {
                        action.getSourceUrl().set(new URL(CleanroomMeta.RESOURCES_BASE_URL + asset.getPath()));
                    } catch (MalformedURLException e) {
                        throw new RuntimeException(e);
                    }
                    action.getSha1().set(asset.hash());
                    action.getTargetFile().set(asset.relativeFile(objectsDir));
                });
            }
        }
    }
